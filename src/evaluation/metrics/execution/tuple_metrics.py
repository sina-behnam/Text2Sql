...
class TupleLevelMetrics(ExecutionBasedMetric):
...
class TupleCardinality(TupleLevelMetrics):
    name = TupleLevelMetricType.TUPLE_CARDINALITY
    description = "Tuple Cardinality Metric"
    ...
    def _compute_score(self, target: ExecutionResult, prediction: ExecutionResult) -> float:
        return self.tuple_cardinality(target.results, prediction.results)
    
class TupleOrder(TupleLevelMetrics):
    name = TupleLevelMetricType.TUPLE_ORDER
    description = "Tuple Order Metric"
    ...
    def tuple_order(target: list[list], prediction: list[list]) -> float | int:
        ...
        return TupleOrder._normalize(round(rho, 3))
    
    def _compute_score(self, target: ExecutionResult, prediction: ExecutionResult) -> float:
        return self.tuple_order(target.results, prediction.results)

class TupleConstraint(TupleLevelMetrics):
    name = TupleLevelMetricType.TUPLE_CONSTRAINTS
    description = "Tuple Constraint Metric"
    ...
    def _sort_with_different_types(arr):
        sorted_arr = sorted(arr, key=TupleConstraint._sort_key)
        return sorted_arr
    ...
    def tuple_constraint(target: list[list], prediction: list[list]) -> float | int:
        ...
        target = [tuple(TupleConstraint._sort_with_different_types(row)) for row in target]
        prediction = [tuple(TupleConstraint._sort_with_different_types(row)) for row in prediction]
    ...
    def _compute_score(self, target: ExecutionResult, prediction: ExecutionResult) -> float:
        return self.tuple_constraint(target.results, prediction.results)