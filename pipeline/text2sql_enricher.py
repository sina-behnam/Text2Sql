...
class Text2SQLInferencePipeline:
    ...
    def task_evaluate(self, instances: List[Tuple[DatasetInstance, str]], output_dir: str = None, do_judge: bool = True):
        """
        ...
        for instance_id, extracted_sql in tqdm(self.extracted_sqls.items(), desc="Evaluating SQL", unit="instance"):
            ...
            # Evaluate the SQL
            evaluation = self.evaluate_instance(instance, extracted_sql, file_path, do_judge=do_judge)
    ...
    def evaluate_instance(self, instance: DatasetInstance, generated_sql: str, instance_path: str, do_judge: bool = True) -> Dict:
        """
        ...
        Args:
            ...
            do_judge: Whether to use the judge model for semantic equivalence checking if needed (default: True)
        Returns:
        ...
        if db_path == 'snowflake':
            self.logger.error("Snowflake database evaluation is not supported in this evaluation method.")
            raise NotImplementedError("Snowflake database evaluation is not supported in this evaluation method.")
    ...
    def run_pipeline(
        ...
        for task in task_enums:
            if task == PipelineTask.GENERATE:
                ...
                self.task_evaluate(instances, output_dir, do_judge=do_judge)
