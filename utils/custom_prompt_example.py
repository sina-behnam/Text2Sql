...
def example_usage():
    """Demonstrates how to use custom templates."""

    # 1. Register your custom templates
    register_prompt_template("codellama-sql-v1", CodeLlamaSQL_FineTunedTemplate)
    register_prompt_template("xml-sql-model", XMLFormatModelTemplate)
    register_prompt_template("healthcare-sql-v2", HealthcareSQLTemplate)

    # 2. Now they can be used automatically in the pipeline
    from utils.prompt_engineering import get_prompt_template

    # Get your custom template
    template = get_prompt_template("codellama-sql-v1")

    # Use it
    question = "Get all patients with diabetes"
    schema = "CREATE TABLE patients (id INT, name TEXT, diagnosis TEXT);"
    ...
    model_response = "SQL: SELECT * FROM patients WHERE diagnosis = 'diabetes'"
    sql = template.extract_sql(model_response)
    print(f"Extracted SQL: {sql}")


# How to use in the Text2SQL pipeline:

def example_pipeline_usage():
    """
    Example of using custom templates in the Text2SQL pipeline.
    """
    from pipeline.text2sql_enricher import Text2SQLInferencePipeline

    # First, register your custom template
    register_prompt_template("my-finetuned-model", CodeLlamaSQL_FineTunedTemplate)

    # Configure the pipeline with your model
    model_config = {
        "type": "local",
        "name": "my-finetuned-model",  # This will use your custom template!
        "path": "/path/to/model",
        "device": "cuda",
        "max_new_tokens": 512
    }

    # Initialize pipeline - it will automatically use your custom template
    pipeline = Text2SQLInferencePipeline(
        model_config=model_config,
        few_shot_examples=[
            {
                "question": "Get all active patients",
                "schema": "CREATE TABLE patients (id INT, status TEXT);",
                "sql": "SELECT * FROM patients WHERE status = 'active'"
            }
        ]
    )

    # The pipeline will now use your custom prompt format and extraction logic!
    # results = pipeline.run_pipeline(instances)


if __name__ == "__main__":
    print("=" * 80)
    print("Custom Prompt Template Examples")
    print("=" * 80)
    example_usage()
